...
task("claim", "", async (taskArgs, hre) => {
  ...
  let claim = async (key) => {
    ...
    if (claimable.isZero()) {
      ...
      await tx.wait();
      console.log(
        `claim ${claimable.div(decimals)} $ARB for ${signer.address}`
      );
    }
    if (taskArgs.receiver !== "") {
      ...
      await tx.wait();
      console.log(
        `transfer ${claimable.div(decimals)} $ARB to ${taskArgs.receiver} for ${
          signer.address
        }`
      );
    }
  ...
  while (true) {
    ...
    if (current < START_BLOCK) {
      console.log(
        `Curent height ${current} waiting for block ${START_BLOCK}...`
      );
      await sleep(3);
      continue;
    }
